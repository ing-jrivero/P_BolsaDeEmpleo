...
namespace ing_soft_ss_07
{
    public partial class Documentos : Form
    {
        ...
        static string R_documentos;
        static string R_carreras;
        static string C_documentos;
        static string D_documentos;
        static int operacion;
        static string tabla = "documento";
        string query;
        ...
        private static void Consulta_b(MySqlConnection conn, string sql, int ope, string tabla)
        {
            ...
            */
                ...
                switch (ope)
                {
                    case 1://Buscar
                        ...
                        while (MyReader.Read())
                        {
                            ...
                            R_documentos = MyReader.GetString("Nombre_" + tabla).ToString();
                        ...
                        break;
                    case 2://Agregar
                        MessageBox.Show("Se ha registrado una nuevo Documento");
                        break;
                    case 3://Modificar
                       /*
                            while (MyReader.Read())
            ...
            else if (tabla.Equals("Chcar_R"))
            {
                while (MyReader.Read())
                {
                    R_documentos = MyReader.GetString("conteo").ToString();
                    MessageBox.Show("Conteo: " + R_documentos);
                    registros = int.Parse(R_documentos);
                ...
                else if (tabla.Equals("Existe_clv"))
                {
                    ...
                    while (MyReader.Read())
                    {
                        ...
                        R_documentos = MyReader.GetString(X).ToString();
        ...
        private void txt_clave_Leave(object sender, EventArgs e)
        {
            operacion = 1;
            //  tabla = "puesto";
            C_documentos = txt_clave.Text;
            if (C_documentos != "")
            {
                query = "Select * from " + tabla + " where Clv_" + tabla + " =" + C_documentos;
                conexion_b(query, operacion, tabla);
                txt_descripcion.Text = R_documentos;
               
            }

            else
            {
                MessageBox.Show("Debes de Introducir Una clave del Documento");
            }
        }
        ...
        private void btn_agragar_Click_2(object sender, EventArgs e)
        {
            operacion = 2;
            //   tabla = "perfil";
            C_documentos = txt_clave.Text;
            D_documentos = txt_descripcion.Text;
            if (D_documentos != "")
            {
                if (C_documentos != "")
                {

                    query = "INSERT INTO " + tabla + "(Clv_" + tabla + ", Nombre_" + tabla + ") VALUES (" + C_documentos + "," + "'" + D_documentos + "'" + ");";
                    conexion_b(query, operacion, tabla);
                    txt_clave.Text = "";
            ...
            {
                MessageBox.Show("Debes de Introducir Una Descripcion del Documento");
            }
        }
        ...
        private void btn_modificar_Click_2(object sender, EventArgs e)
        {
            ...
            if (D_documentos != "")
            {
                if (C_documentos != "")
                {
                    if (existe(C_documentos, "Clv_" + tabla))
                    {
                        //    UPDATE materias SET nombre = 'Ingenieria de Software2222'WHERE ClvMateria = 001;
                        query = "UPDATE " + tabla + " SET nombre_" + tabla + " = " + "'" + D_documentos + "'" + " Where Clv_" + tabla + " = " + C_documentos + ";";
                        conexion_b(query, operacion, tabla);
                        //txt_descripcion.Text = R_documentos;
                        if (est == true)
                        {
        ...
        private void btn_borrar_Click_2(object sender, EventArgs e)
        {
            operacion = 4;
            // tabla = "perfil";
            C_documentos = txt_clave.Text;
            ...
            if (D_documentos != "")
            {
                if (C_documentos != "")
                {
                    ...
                    query = "DELETE FROM " + tabla + " WHERE Clv_" + tabla + " = " + C_documentos + ";";
                    conexion_b(query, operacion, tabla);
